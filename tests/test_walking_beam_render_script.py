...
def test_render_script_help_documents_artifacts_and_overrides() -> None:
    ...
    for expected in (
        ...
        "walking_beam_alignment_top_down.mp4",
...
def test_render_script_dry_run_lists_full_movie_pipeline(tmp_path: Path) -> None:
    ...
    for expected in (
        ...
        str(output_dir / "frames/top_down/frame_"),
        ...
        str(output_dir / "walking_beam_alignment_top_down.mp4"),
        "Render mode: final",
        "Cycles device: default",
        "Wide Setup Camera",
        "Iris Close-Up Camera",
        "Hero Camera",
        "Top-Down Path Camera",
        "vstack=inputs=3",
    ):
