...
This document describes how to properly connect and configure the Tiger Controller
to drive the voice coil mirror through the servo amplifier.

This setup prevents image blur caused by mirror oscillation and ensures stable scanning.
...
- The internal low-pass filter on the DAC must be enabled.
- The signal waveform must avoid abrupt transitions.
...
The servo amplifier has two command input terminals:
...
| (Reference)          |                       |
+----------------------+-----------------------+
| Analog Ground        | Servo Analog Ground   |
+----------------------+-----------------------+

Ensure:

- All connections are secure.
- Analog ground is connected only to the servo's analog ground terminal.
- Do not leave command inputs floating.
...
- Instantaneous flyback transitions
- Sharp waveform corners
- Step changes in position

If using ramp or triangle wave scanning:

- Use a finite fall time.
- Avoid instantaneous return-to-start.
...
4. Symptoms of Incorrect Configuration
--------------------------------------

+-----------------------------+------------------------------------+
| Symptom                     | Likely Cause                       |
+=============================+====================================+
| Image blur during flyback   | Fall time too short                |
+-----------------------------+------------------------------------+
| Visible mirror oscillation  | Filter disabled or cutoff too high |
+-----------------------------+------------------------------------+
| Small ripple during scan    | Mismatched filter settings         |
+-----------------------------+------------------------------------+

Correct configuration eliminates visible oscillation.
...
- [ ] Image stable during ramp motion
