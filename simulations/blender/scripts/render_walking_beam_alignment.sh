...
usage() {
  cat <<'EOF'
Render the walking-beam alignment Blender scene as teaching movies.

Usage:
  simulations/blender/scripts/render_walking_beam_alignment.sh [--draft|--preview|--final] [--dry-run] [OUTPUT_DIR]
  simulations/blender/scripts/render_walking_beam_alignment.sh --help
...
Examples:
  ...
  simulations/blender/scripts/render_walking_beam_alignment.sh --draft output/draft
  simulations/blender/scripts/render_walking_beam_alignment.sh --preview output/preview
  FRAME_START=1 FRAME_END=24 RESOLUTION_X=960 RESOLUTION_Y=540 \
    simulations/blender/scripts/render_walking_beam_alignment.sh --preview output/smoke
EOF
}
...
output_dir="${output_dir:-output/walking_beam_alignment}"
case "$output_dir" in
  /*) ;;
...
Would run:
  ...
  Render Iris Close-Up Camera frames to $draft_frame_prefix
...
if [[ "$render_mode" == "draft" ]]; then
  render_view "Iris Close-Up Camera" "$draft_frame_prefix"
